...
- `npm test` — Karma/Jasmine in Chrome. Single spec: `npx ng test --include=src/app/components/pages/stock-productos/stock-productos.component.spec.ts`.
- Deploy: `angular-cli-ghpages` is a devDependency (gh-pages publishing of `dist/pos`).

Angular 14 with modules (no standalone components / signals). TypeScript strict mode is on (`tsconfig.json`), so new code must satisfy strict null checks.

## Architecture

Angular 14 + Angular Material POS (point-of-sale) front-end for a .NET backend. UI text, identifiers, and folder names are in Spanish — keep that convention.
...
- `ReusableModule` is the shared barrel — it imports/exports all Angular Material modules plus `HttpClientModule`, `FormsModule`, `ReactiveFormsModule`. Import it rather than adding Material imports per-module.
